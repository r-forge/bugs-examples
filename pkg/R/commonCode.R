makeExample <- function(name, parameters, nIter=20000, nBurnin=5000, nThin=1, nChain=1)
{
...
Air <- makeExample("Air", c("X","theta" ), nIter=50000, nBurnin=10000)
Alligators <- makeExample("Alligators", c("b", "beta", "g", "gamma"), nIter=50000, nBurnin=10000)
...
Camel <- makeExample("Camel", c("Sigma2", "rho", "tau"), nIter=101000, nThin=10)
...
Dyes <- makeExample("Dyes", c("sigma2.btw","sigma2.with", "theta"), nIter=125000, nThin=10, nBurnin=25000)
Endo <- makeExample("Endo", "beta", nIter=15000, nBurnin=5000)
...
HepatitisME <- makeExample("HepatitisME", c("alpha0", "beta0", "gamma", "sigma"), nIter=50000, nBurnin=10000)
...
Ice <- makeExample("Ice",c("logRR", "sigma"), nIter=101000, nThin=10)
Jama <- makeExample("Jama", c("alpha.desc", "beta.desc"), nIter=15000, nBurnin=5000)
Jaws <- makeExample("Jaws", c("beta0", "beta1", "mu", "Sigma"))
Kidney <- makeExample("Kidney", c("alpha", "beta.sex", "beta.dis", "r", "sigma"), nIter=50000, nBurnin=10000)
...
Magnesium <- makeExample("Magnesium", c("OR", "tau"), nIter=5000)
Mice <- makeExample("Mice", c("median", "pos.control", "r", "test.sub", "veh.control"))
OtreesMVN <- makeExample("OtreesMVN", c("mu", "sigma", "sigmaC"), nIter=50000, nBurnin=5000)
Otrees <- makeExample("Otrees", c("mu", "sigma", "sigmaC"), nIter=30000, nBurnin=10000)
Oxford <- makeExample("Oxford", c("alpha", "beta1", "beta2", "sigma"))
Pigs <- makeExample("Pigs", c("i", "p"), nIter=110000, nThin=10, nBurnin=10000)
...
StVeit <- makeExample("StVeit", "within", nIter=50000, nBurnin=10000)
...
Scotland <- makeExample("Scotland", c("RR","alpha0","alpha1","sigma"), nIter=50000, nBurnin=10000)
LHA <- makeExample("LHA", c("RR","alpha","beta","sigma.b","sigma.h"), nIter=100000, nThin=4, nBurnin=20000)
Scotland1 <- makeExample("Scotland1", c("RR","alpha","sigma"))
Elevation <- makeExample("Elevation", c("height.pred","height.pred.multi"))
Forest <- makeExample("Forest", c("theta0","theta1","theta2","spatial.effect"), nIter=20000, nBurnin=10000)
Huddersfield <- makeExample("Huddersfield", c("PC.base","PC.latent","PC.no2","rate.base","rate.latent","rate.no2"),
                         nIter=60000, nBurnin=30000)
MVCAR <- makeExample("MVCAR", c("alpha","corr","sigma"), nIter=51000, nBurnin=1000)
